...
def get_ctc_vocab(char_list, add_blank = True):
    # blank
    if add_blank :
        ctc_char_list = ['BLNK'] + list(char_list)
    else :
        ctc_char_list = list(char_list)
    ctc_map, inv_ctc_map = {}, {}
...
def get_vocab(char_list):
    char_list = ['PAD', 'SOS', 'EOS'] + list(char_list)
    map, inv_map = {}, {}
    for i, char in enumerate(char_list):
        map[char] = i
        inv_map[i] = char
    return map, inv_map, char_list
