...
def train(rnn, optimizer, train_data, batch_size, vocab_map, device):
    ...
    for i in range(0, len(perm), batch_size):
        ...
        for j in perm[i: i+batch_size]:
            input.append([vocab_map["SOS"]]+train_data[perm[j]]+[vocab_map["EOS"]])
            grt.append(input[-1][1:]+[vocab_map["EOS"]])
        input, mask = utils.make_batch(input, vocab_map['EOS'])
        grt, _ = utils.make_batch(grt, vocab_map['EOS'])
...
def test(rnn, test_data, vocab_map, device):
    ...
    for i in range(0, len(perm), test_batch_size):
        ...
        for j in perm[i: i+test_batch_size]:
            input.append([vocab_map["SOS"]]+test_data[perm[j]]+[vocab_map["EOS"]])
            grt.append(input[-1][1:]+[vocab_map["EOS"]])
        input, mask = utils.make_batch(input, vocab_map['EOS'])
        grt, _ = utils.make_batch(grt, vocab_map['EOS'])
        with torch.no_grad():
...
def main():
    ...
    char_list = list(config['LANG']['chars']) + ['SOS', 'EOS'] + ['PAD']
