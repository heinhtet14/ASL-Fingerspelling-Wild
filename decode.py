...
def main():
    ...
    for subdir, label in labels.items():
        ...
        lb_arr.append(list(map(lambda x: vocab_map[x], label)))
        ...
        if args.decode_type == "greedy":
            ...
            #pred = decoder.beam_decode(prob, beam_size=args.beam_size, beta=args.lm_beta, gamma=args.ins_gamma, digit=True)
        else:
